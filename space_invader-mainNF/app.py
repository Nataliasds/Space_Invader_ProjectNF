...
def show_blife(x, y):
    life = FONT.render("Boss Life: " + str(b_life_bar), True, (255, 255, 255))
...
def message_display(text):
    victory = VICTORY_FONT.render(text, True, (255, 255, 255))
    SCREEN.blit(victory, (270, 250))
...
def isCollisionBoss(bossX, bossY, bulletX, bulletY):
    distance = math.sqrt(
        (math.pow(bossX - bulletX, 2)) + (math.pow(bossY - bulletY, 2))
    )
    if distance < 27:
        return True
    return False
...
while running:
    ...
    if score_value == 5:
        ...
        bossY = 50
        boss(bossX,bossY)
        show_blife(BTEST_X,BTEST_Y)

        collisionboss = isCollisionBoss(bossX + 5, bossY + 5, bulletX, bulletY)  
        if collisionboss:
            bulletY = 480
            bullet_state = "ready"
            boss_health -= 1
